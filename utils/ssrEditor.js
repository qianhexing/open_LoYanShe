...
const BlockEmbed = _quill.import('blots/embed')
const BlockEmbed1 = _interopRequireDefault(BlockEmbed);
class editorLibrary extends BlockEmbed  {
  static create(e) {
...
editorLibrary.blotName = 'editorLibrary';
editorLibrary.tagName = 'editorLibrary';
// 投票
class lolitaVote extends BlockEmbed  {
  static create(e) {
    let node = super.create();
    let iframe = document.createElement("iframe");
    iframe.src = 'https://lolitalibrary.com/vote/' + e.vote_id
    iframe.setAttribute('style', 'border:none; width:100%;')
    iframe.onload = () => {
      this.height = this.contentWindow.document.body.scrollHeight
    }
    iframe.name = 'vote_' + e.vote_id
    iframe.id = 'vote_' + e.vote_id
    iframe.setAttribute('scrolling', 'no')
    iframe.setAttribute('frameborder', '0')
    iframe.className = 'lolita-vote'
...
lolitaVote.blotName = 'lolitaVote';
lolitaVote.tagName = 'lolitaVote';
_quill.register(editorLibrary);
_quill.register(lolitaVote);
